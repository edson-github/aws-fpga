...
class TestBuildSDAccelExample(AwsFpgaTestBase):
    ...
    def check_build(self, examplePath, target):
        ...
        logger.info(f"Checking if SDAccel Example xclbin path={xclbin_path} exists")
        assert os.path.exists(
            xclbin_path
        ), f"SDAccel Example xclbinpath={xclbin_path} does not exist"

        logger.info(
            f"Checking that a non zero size xclbin file exists in {xclbin_path}"
        )
        xclbin = self.assert_non_zero_file(
            os.path.join(xclbin_path, f"*.{target}.*.xclbin")
        )
    ...
    def base_test(self, examplePath, target, rteName, xilinxVersion, clean=True, check=True):
        ...
        if clean:
            ...
            assert rc == 0, f"SDAccel build failed while cleaning with rc={rc}"

        check_string = "check" if check else ""
        ...
        assert rc == 0, f"SDAccel build failed with rc={rc}"
