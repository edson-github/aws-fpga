...
class TestFindSDAccelExamples(AwsFpgaTestBase):
    ...
    def test_find_example_makefiles(self, xilinxVersion):
        ...
        assert os.path.exists(
            self.xilinx_sdaccel_examples_dir
        ), f"The Xilinx SDAccel example dir does not exist: {self.xilinx_sdaccel_examples_dir}"
                ...
                with open(f"{root}/description.json", "r") as description_file:
                    ...
                    if "nboard" in description:
                        if "xilinx_aws-vu9p-f1-04261818" in description["nboard"]:
                            ignore = True
                            logger.info(f"Ignoring {root} as F1 device found in nboard.")
                            continue
            else:
                ignore = True
        ...
        assert (
            xilinx_examples_makefiles
        ), f"Could not find any Xilinx SDAccel example in {self.xilinx_sdaccel_examples_dir}"
        ...
        with open(f"{self.xilinx_sdaccel_examples_list_file}.{xilinxVersion}", 'w') as archive_file:
            json.dump(xilinx_sdaccel_example_map, archive_file)

        assert (
            os.path.getsize(self.xilinx_sdaccel_examples_list_file) > 0
        ), f"{self.xilinx_sdaccel_examples_list_file} is a non zero file. We need to have some data in the file"
