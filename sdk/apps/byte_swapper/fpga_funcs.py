...
class FPGAFuncs(object):
    ...
    def get_slot_info(self):
        logger.info(f"Retrieving slot info (slot={self.slot}).")
        ret = c_int32()
        ...
        if ret != 0:
            logger.error(f"Unable to describe current image (returned {ret})")
    ...
    def load_afi(self):
        info = self.get_slot_info()
        ret = c_int32()
        logger.info(f"Current AFI: {info.ids.afi_id}")
        ...
        if ret != 0:
            raise RuntimeError(f"Could not load AFI {self.AFI}")
        logger.info(f"Loaded AFI: {info.ids.afi_id}")
